...
@section('content')
    ...
    <div class="card shadow mb-4">
        <div class="card-body">
            <form action="{{route('admin.products.postUpdate',['id'=>$id])}}" method="POST" enctype="multipart/form-data">
                <div style="display: grid; grid-template-columns: repeat(2, 1fr)">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="amount" class="form-control w-50" placeholder="Product Amount" min="1" max="1000" value="{{old('amount') ?? $product->amount}}">
                        @error('amount')
                            <span style="color: red">{{$message}}</span>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="file" name="image" class="form-control w-50" placeholder="Product Imgae..." value="{{old('image') ?? $product->image}}">
                        @error('image')
                            <span style="color: red">{{$message}}</span>
                        @enderror
                    </div>
