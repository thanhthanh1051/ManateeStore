...
class DiscountController extends Controller
{
    ...
    public function postAdd(Request $request){
        ...
        $check = $discount->save();
        if($check){
            return redirect()->route('admin.discounts.getList',compact('check'));
        }
        return redirect()->route('admin.discounts.getList')->with('msg','Error');
    }
    ...
    public function deleteItem($id=0){
        if(!empty($id) && ctype_digit($id)){
            ...
            if(!empty($discount)){
                ...
                return redirect()->route('admin.discounts.getList')->with('msg','Success');
            }
        }
        return redirect()->route('admin.discounts.getList')->with('msg','Error');
    }
